...
func (r *runner) safeRun(host string, fn func(host string)) {
	defer func() {
		// don't panic
		err := recover()
		if err != nil {
...
func (r *runner) spawnWorkers(host string, spawnCount int, quit chan bool, hatchCompleteFunc func()) {
	log.Printf("Hatching and swarming %d clients at the rate %.2f clients/s, targeting %q", spawnCount, r.hatchRate, host)
				...
				for {
					...
					default:
						if r.rateLimitEnabled {
							...
							if !blocked {
								r.safeRun(r.host, task.Fn)
							}
						} else {
							r.safeRun(host, task.Fn)
						}
					}
				}
			}()
...
func (r *runner) startHatching(host string, spawnCount int, hatchRate float64, hatchCompleteFunc func()) {
	...
	go r.spawnWorkers(host, spawnCount, r.stopChan, hatchCompleteFunc)
}
...
func (r *localRunner) run() {
	...
	r.startHatching(r.host, r.hatchCount, r.hatchRate, nil)
...
func (r *slaveRunner) onHatchMessage(msg *message) {
	...
	r.startHatching(target, workers, hatchRate, r.hatchComplete)
}
